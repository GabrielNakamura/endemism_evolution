...
DivB_metrics <-function(W,
  ...
  if(PD == TRUE | PE == TRUE){
    ...
    if(PE == TRUE){
      ...
      for(i in 1:length(list_matrix_edges)){
        if(any(is.null(dim(list_matrix_edges[[i]]))) == TRUE){
          list_occurence[[i]]<- NA
        } else {
          ...
          for(j in 1:length(posit)){
            if (any(is.na(list_matrix_edges[[all_comb[posit[j], 2]]])) == TRUE){
              comb_occ[, j]<- FALSE
            } else {
      ...
      for(i in 1:length(list_matrix_edges_AS)){
        if(any(is.na(list_brLength_divLocal[[i]])) == TRUE){
          ...
          for(j in 1:length(a)){
            ...
            denom_tmp2[j]<- sum(list_edge_brLen_insitu[[i]][j, ][!is.na(match(names(list_edge_brLen_insitu[[i]][j, ]), a[[j]]))])
            } else{
       ...
       list_all_DbInfo[[i]] <- cbind(list_edge_brLen_insitu[[i]][,1], ((denom_tmp2) + 1))
      }
