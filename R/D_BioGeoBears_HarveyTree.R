# Before installing BioGeoBears, install dependencies
require(Rcpp)
...
# Cut tree to species of Tyrannidae in the sample
require(geiger)
...
geofile <- read_PHYLIP_data(here::here("data", "Phyllip_GeoAreas.txt"))
geogfn = geofile

# the location # the path to the file is the object to use, the the file itself
geogfn = np(paste(here::here("data", "Phyllip_GeoAreas.txt", sep="")))
moref(geogfn)
...
#### RUN DEC + J ####
resfn = "Alina_DEC+J_M0_unconstrained.Rdata"
runslow = TRUE
if (runslow)
{
...
#### RUN DIVALIKE ####
runslow = TRUE
resfn = "Alina_DIVALIKE_M0_unconstrained.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
#### RUN DIVALIKE +J ####
resfn = "Alina_DIVALIKE+J_M0_unconstrained.Rdata"
runslow = TRUE
if (runslow)
{
  #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
...
#### RUN BAYAREALIKE ####
runslow = TRUE
resfn = "Alina_BAYAREALIKE_M0_unconstrained.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
#### RUN BAYAREALIKE + J ####
resfn = "Alina_BAYAREALIKE+J_M0_unconstrained.Rdata"
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
